...
export default function WelcomeScreen({
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.canvas, paddingHorizontal: 22, paddingBottom: 30 }}>
      ...
      <View style={{ gap: 10 }}>
        <MetallicButton label='Continue with KingsChat' onPress={() => onLogin && onLogin('kingschat')} disabled={busy} />
        <GhostButton height={52} label='Continue with Google' onPress={() => onLogin && onLogin('google')} loading={pending === 'google'} disabled={busy} />
        <GhostButton height={52} label='Continue with email' onPress={() => onLogin && onLogin('email')} loading={pending === 'email'} disabled={busy} />
