...
import { Screen, Card, Label, Mono, Body, Display, MetallicButton, ProgressBar, BackChevron } from '../components/ui';

// Design 3b: earn space — KSH balance, claimable points, weekly settlement, bounties.
const kash = { balance: '128.40', fiat: '≈ $173.84 · ₦267,700', points: '1,240 pts', pointsUsd: '· ≈ $16.90', gatePct: 62 };
const bounties = [
...
export default function EarnSpaceScreen({ onBack }: { onBack?: () => void }) {
  return (
    <Screen>
      ...
      <Card style={{ marginTop: 18, borderRadius: 22, padding: 18, borderColor: 'rgba(255,255,255,0.14)' }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <Mono size={10} color={C.accent} style={{ letterSpacing: 2 }}>◆ KASH</Mono>
          <Mono size={9.5} color={C.dim}>KSH · ON-CHAIN</Mono>
        </View>
        <Body size={11} color={C.dim} style={{ marginTop: 14 }}>KSH balance</Body>
        <Display size={34} style={{ marginTop: 4 }}>{kash.balance} <Display size={18} color={C.sub}>KSH</Display></Display>
        <Mono size={12} color={C.sub} style={{ marginTop: 5 }}>{kash.fiat}</Mono>
        <View style={{ height: 1, backgroundColor: 'rgba(255,255,255,0.1)', marginVertical: 14 }} />
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          <View>
            <Body size={11} color={C.dim}>Claimable points</Body>
            <Body size={14.5} semibold style={{ marginTop: 3 }}>{kash.points} <Mono size={11.5} color={C.sub}>{kash.pointsUsd}</Mono></Body>
          </View>
          <View style={{ width: 86 }}><MetallicButton label='Claim' height={36} radius={11} size={12.5} /></View>
        </View>
      ...
      <View style={{ marginTop: 18 }}>
        ...
        {bounties.map((b, i) => (
          <View key={b.title} style={{ flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 13, borderBottomWidth: i === bounties.length - 1 ? 0 : 1, borderBottomColor: C.hairline }}>
            ...
            <View style={{ flex: 1 }}>
              <Body size={13.5} semibold>{b.title}</Body>
              <Body size={11} color={C.dim} style={{ marginTop: 2 }}>{b.sub}</Body>
            </View>
            <Mono size={11.5} color={C.accent} style={{ backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 8, paddingVertical: 5, paddingHorizontal: 10, overflow: 'hidden' }}>{b.pts}</Mono>
          </View>
        ))}
      </View>
    </Screen>
