  ...
  return (
    <Screen>
      ...
      >
        ...
        >
          <Text style={{ fontFamily: F.display, fontSize: 16, color: C.text }}>
            {user.initial}
          </Text>
        </Pressable>
        <View style={{ flex: 1 }}>
          ...
          <Body size={14.5} semibold>
            Denga{" "}
            <Body size={14.5} color={C.dim}>
              · {user.tag}
            </Body>
          </Body>
        </View>
      ...
      >
        {hubTiles.map((t) => (
          <Pressable
            key={t.key}
            onPress={() => onOpen && onOpen(t.key)}
            style={{ width: "48%", flexGrow: 1 }}
          >
            <Card
              style={{
                padding: 15,
                borderColor: t.badge ? C.borderStrong : C.border,
              }}
            >
              ...
              <Body size={13} semibold style={{ marginTop: 12 }}>
                {t.title}
              </Body>
              {"powered" in t && t.powered ? (
                <Mono
                  size={9}
                  color={C.dim}
                  style={{ marginTop: 3, letterSpacing: 1.2 }}
                >
                  POWERED BY {String(t.powered)}
                </Mono>
              ) : null}
              <Mono
                size={15}
                color={C.text}
                style={{ marginTop: 7, fontFamily: F.monoSemibold }}
              >
                {t.value}
                {t.delta ? (
                  <Mono size={10.5} color={C.up}>
                    {" "}
                    {t.delta}
                  </Mono>
                ) : null}
              </Mono>
              <Body size={10} color={C.dim} style={{ marginTop: 4 }}>
                {t.sub}
              </Body>
            </Card>
          </Pressable>
      ...
      >
        Move money between spaces anytime — one ledger underneath.
      </Body>
    </Screen>
  );
}
