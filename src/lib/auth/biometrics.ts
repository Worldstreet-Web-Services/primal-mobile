...
export async function getCapability(): Promise<BiometricCapability> {
  ...
  if (__DEV__) {
    // Diagnostic only — nothing here changes what this function returns.
    // `available` is the single input to whether the lock screen offers a
    // biometric button, and when it comes back false there is no way to tell
    // from the UI which of the three answers was the no. Delete freely.
    console.log("[biometrics] capability", { hasHardware, isEnrolled, types });
  }
