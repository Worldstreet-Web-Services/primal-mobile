...
import { Screen, Card, MetallicButton, Mono, Body, BackChevron } from '../components/ui';

// Design 3d: The Last Man — pot, countdown, last-man row, pending claim, metallic CTA.
const round = { pot: '$1,240', clock: '04:32', lastMan: '@tobi', lastManInitials: 'TO', entry: '$50' };
  ...
  return (
    <Screen>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingTop: 10 }}>
        ...
        <Mono size={11} color={C.sub} style={{ letterSpacing: 2 }}>THE LAST MAN · ROUND #63</Mono>
      </View>
      <View style={{ marginTop: 40, alignItems: 'center' }}>
        <Mono size={10} color={C.dim} style={{ letterSpacing: 2.5 }}>IN THE POT</Mono>
        <Svg width='100%' height={70} style={{ marginTop: 8 }}>
          ...
          <SvgText x='50%' y={58} textAnchor='middle' fontFamily={F.display} fontSize={64} fill='url(#pot)'>{round.pot}</SvgText>
        </Svg>
        <Text style={{ marginTop: 26, fontFamily: F.monoSemibold, fontSize: 48, letterSpacing: 2, color: C.amber }}>{round.clock}</Text>
        <Body size={11.5} color={C.dim} style={{ marginTop: 6 }}>until the pot pays out</Body>
      </View>
      <Card style={{ marginTop: 30, flexDirection: 'row', alignItems: 'center', gap: 12, borderRadius: 18, paddingVertical: 14, paddingHorizontal: 16 }}>
        <View style={{ width: 42, height: 42, borderRadius: 22, backgroundColor: 'rgba(245,184,61,0.14)', alignItems: 'center', justifyContent: 'center' }}>
          <Text style={{ fontFamily: F.display, fontSize: 14, color: C.amber }}>{round.lastManInitials}</Text>
        </View>
        <View style={{ flex: 1 }}>
          <Body size={14} semibold>{round.lastMan} is the last man</Body>
          <Body size={11} color={C.dim} style={{ marginTop: 2 }}>Took the lead 28s ago · 14th entry this round</Body>
        </View>
      </Card>
      ...
      <View style={{ marginTop: 44 }}>
        <Body size={11} color={C.dim} style={{ textAlign: 'center', lineHeight: 17.5, marginBottom: 14 }}>{'Pay the $50 entry to become the last man and reset the clock.\nIf it hits zero while you’re last — the whole pot is yours.'}</Body>
