...
export default function GamesSpaceScreen({
  ...
  return (
    <Screen top={top}>
      ...
      <Pressable
        onPress={() => onOpenGame && onOpenGame("last-man")}
        style={{ marginTop: 14 }}
      >
        <Card
          style={{ borderRadius: 24, padding: 20, borderColor: C.borderStrong }}
        >
          <View
            style={{
              flexDirection: "row",
          ...
          >
            ...
            >
              <PulseDot />
              <Body size={10} color={C.amber}>
                LIVE
              </Body>
            </View>
          </View>
          <Display size={26} style={{ marginTop: 12, lineHeight: 30 }}>
            {"The Last Man\nStanding"}
          </Display>
          <View style={{ marginTop: 14, flexDirection: "row", gap: 22 }}>
            {flagshipStats.map((s) => (
              <View key={s.k}>
                <Mono size={9.5} color={C.dim} style={{ letterSpacing: 1 }}>
                  {s.k}
                </Mono>
                <Mono
                  size={16}
                  color={s.amber ? C.amber : C.text}
                  style={{ marginTop: 3, fontFamily: F.monoSemibold }}
                >
                  {s.v}
                </Mono>
              </View>
          ...
          <Body size={10.5} color={C.dim} style={{ marginTop: 10 }}>
            Pay in, take the lead, reset the clock. Outlast everyone — the pot
            is yours.
          </Body>
        </Card>
      </Pressable>
      <View style={{ marginTop: 14, flexDirection: "row", gap: 11 }}>
        <Card style={{ flex: 1, borderRadius: 18, padding: 16 }}>
          <Text style={{ fontSize: 26, color: C.accent, lineHeight: 28 }}>
            ♞
          </Text>
          <Body size={14} semibold style={{ marginTop: 10 }}>
            Chess
          </Body>
          <View
            style={{
      ...
      >
        <Body size={11.5} color={C.dim} style={{ textAlign: "center" }}>
          More games land on this shelf soon
        </Body>
      </View>
      <Body
        size={11}
      ...
      >
        Plays are on-chain from your crypto balance · signed with Face ID
      </Body>
      <Body
        size={11}
        color={C.dim}
      ...
      >
        Outlast the clock — powered by Ark
      </Body>
    </Screen>
  );
}
